...
if arquivo_carregado is not None:
    ...
    try:
        df = carregar_dados('/home/ubuntu/df_diarios.csv')
...
with col2:
    ...
    bins = [0, 0.5, 1, 2, 5, df_filtrado['ip_d'].max()]
    labels = ['0-0.5', '0.5-1', '1-2', '2-5', '>5']
    df_filtrado['ip_bin'] = pd.cut(df_filtrado['ip_d'], bins=bins, labels=labels)

    dist = df_filtrado['ip_bin'].value_counts().sort_index()
...
st.markdown("""
    ...
    **Dashboard de Produtividade** | Desenvolvido com Streamlit
    ...
    Dados: Análise de mão de obra em obras de construção civil
""")